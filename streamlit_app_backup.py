...
def render_ai_assistant():
    """AI Sales Assistant Chat Interface"""
    st.title("🤖 AI Sales Assistant")
    st.markdown("Your AI-powered sales expert using MCP integration")
    
    # Initialize chat history
    if "messages" not in st.session_state:
        st.session_state.messages = [
            {"role": "assistant", "content": "Hello! I'm your AI Sales Assistant. Try asking:\n- 'Tell me about TechCorp'\n- 'What's my forecast?'\n- 'Draft a follow-up email'"}
        ]
    ...
    # Chat input
    if prompt := st.chat_input("Ask your AI Sales Assistant..."):
            ...
            with st.spinner("Thinking..."):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        assistant_message = "Error connecting to AI service."
...
# Main content area
if st.session_state.view_mode == 'ai_assistant':
    render_ai_assistant()
...
elif st.session_state.view_mode == 'integrations':
    # Integrations Page
    st.markdown('<h1 class="dashboard-header">🔌 Integrations Hub</h1>', unsafe_allow_html=True)
    st.markdown("Connect your sales tools and automate workflows")
    ...
    try:
        ...
        status = {"gmail": False, "voice": False, "available": False, "note": "API not available"}
    ...
    with tab1:
        ...
        if st.button("🔄 Refresh Emails", key="refresh_emails"):
            with st.spinner("Fetching emails..."):
                try:
                    response = requests.get(f"{API_BASE}/integrations/gmail/unread")
                    if response.status_code == 200:
            ...
            for email in st.session_state['emails']:
                with st.expander(f"📧 {email['subject']} - From: {email['from']}"):
                    ...
                    with col1:
                        ...
                        if intent == "buying":
                            st.metric("Intent Level", "High", f"Score: {score}", delta_color="normal")
                        elif intent == "interested":
                            st.metric("Intent Level", "Medium", f"Score: {score}", delta_color="normal")
                        else:
                            st.metric("Intent Level", "Low", f"Score: {score}", delta_color="off")

                    with col2:
                        urgency = analysis.get('urgency', 'Unknown')
                        if urgency == "high":
                            st.metric("Urgency", urgency.title(), delta_color="inverse")
                        else:
                    ...
                    suggestions = [
                        "Schedule a demo call within 24 hours",
    ...
    # Voice Coaching Tab
    with tab2:
        st.header("Voice Coaching & Real-time AI")

        col1, col2 = st.columns(2)

        with col1:
            st.subheader("🎙️ Start a Call")
            participant = st.text_input("Participant Name", "John Doe")
            
            if st.button("📞 Start Call", type="primary"):
                try:
                    response = requests.post(f"{API_BASE}/integrations/voice/start-call", 
                                          json={"participant": participant})
                    if response.status_code == 200:
                        call_data = response.json()
                        st.session_state['current_call'] = call_data['call_id']
                        st.success(f"Call started! ID: {call_data['call_id']}")
                except Exception as e:
        ...
        with col2:
            ...
            if 'current_call' in st.session_state:
                st.info(f"Active Call: {st.session_state['current_call']}")
                
                # Mock transcript
                transcript = [
                    {"speaker": "You", "text": "Hi John, thanks for taking the time today."},
                    {"speaker": "John", "text": "Thanks for reaching out. We're looking for a solution."},
                    {"speaker": "You", "text": "Great! Let me show you how we can help."}
                ]
                ...
                coaching = [
                    "✅ Good opening - establish rapport",
                    "⚠️ Ask about their specific pain points",
